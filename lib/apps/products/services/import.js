...
var createProduct = function(product, info, brand, image) {
  return Product.listByName(product).then(function(p) {
    if (p.length === 0) return Product.create({ name: product, info: info, brand: brand.id, image: image });
    return p[0];
  });
};
...
var create = function(data) {
  ...
  return createBrand(brand).then(function(brand) {
    return createProduct(product, info, brand, image).then(function(product) {
...
var imp = module.exports = Promise.method(function() {
  ...
  return readFile(path.join(__dirname, '../data/2021.csv'), 'utf8').then(function(data) {
