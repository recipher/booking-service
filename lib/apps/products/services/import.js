...
var createProduct = function(product, info, image, brand) {
  return Product.listByName(product).then(function(p) {
    if (p.length === 0) return Product.create({ name: product, info: info, image: image, brand: brand.id });
    return p[0];
  });
};
...
var createModel = function(model, product) {
  return Model.listByNameForProduct(product.id, model).then(function(m) {
    if (m.length === 0) return Model.create({ name: model, product: product.id });
    return m[0];
  });
};
...
var create = function(data) {
  ...
  return createBrand(brand).then(function(brand) {
    return createProduct(product, info, image, brand).then(function(product) {
...
var imp = module.exports = Promise.method(function() {
  ...
  return readFile(path.join(__dirname, '../data/products.csv'), 'utf8').then(function(data) {
