...
var email = function(bookings, payment, user, host) {
  if (bookings == null || bookings.length === 0) return;
  
  var data = { bookings: _.map(bookings, describe), payment: payment };
  ...
  return Booking.listByMember(member.id).then(function(existing) {
    ...
    .then(function(bookings) {
      ...
      if (payment == null || payment.amount === 0) {
        email(bookings, { amount: 0 }, context.session, context.host);
        return bookings;
      }
      ...
      .then(function(payment) {
        email(bookings, payment, context.session, context.host)
        return bookings;
      })
